...
async def main():
    ...
    forecast_agent = ForecastAgent(
        name="ForecastAgent",
        description="Generates revenue forecasts using ARIMA",
        ontology_loader=ontology_loader,
    )
    task1 = AgentTask(
        description="Forecast Q1-Q3 revenue for Sunshine Bakery",
        parameters={"business_name": "Sunshine Bakery"},
    )
    ...
    if OPENAI_SDK_AVAILABLE:
        ...
        task2_description = (
            f"Based on this forecast: '{result1.action_result.summary}', "
            "extract structured revenue insight for Q2."
        )
    ...
    task3 = AgentTask(
        description="Optimize revenue based on forecast trends", parameters={}
    )
