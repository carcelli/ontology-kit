...
async def main():
    ...
    add_ontology_statement(
        new_insight_id, "hasText", new_insight_text, object_type="literal"
    )
    add_ontology_statement(
        new_insight_id, "informsBusiness", affected_business_id, object_type="uri"
    )

    print(
        f"Added new insight '{new_insight_id}': '{new_insight_text}' informing '{affected_business_id}'"
    )

    # 3. Verify the new insight is in the ontology
    print("\n--- Verifying new insight ---")
    verify_query_results = query_ontology(
        f"""
        PREFIX : <http://agent_kit.io/business#>
        SELECT ?text WHERE {{
