...
def main():
    ...
    if os.path.exists(ontology_path):
        ...
        for lever in result1["top_levers"][:5]:
            ...
            print(
                f"    - Sensitivity:   {breakdown['sensitivity']:.2f} (dist to Revenue)"
            )
            print(
                f"    - Uncertainty:   {breakdown['uncertainty']:.2f} (cluster variance)"
            )
    ...
    if "CustomerSatisfaction" in business_terms:
        ...
        for term in actionable[:5]:
            rev_score = next(
                (l["leverage"] for l in result2["top_levers"] if l["term"] == term), 0.0
            )
            sat_score = next(
                (l["leverage"] for l in result3["top_levers"] if l["term"] == term), 0.0
            )
    ...
    print("\n📁 Generated Files:")
    for f in [
        "business_leverage.png",
        "business_intervention_3d.png",
        "satisfaction_leverage.png",
    ]:
        path = f"outputs/{f}"
