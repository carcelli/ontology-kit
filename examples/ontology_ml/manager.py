...
async def main(
    ...
    onto_path = create_or_update_ontology(
        json.dumps(proposal.model_dump()), output_path=DEFAULT_ONTOLOGY
    )
    ...
    print("-" * 70)
    terms = ", ".join(
        sorted({p.name for p in proposal.properties} | {"hasDate", "hasTotal"})
    )
    mapper_prompt = (
    ...
    rdf = map_csv_to_rdf(
        csv_path, mapping_json, output_path="examples/ontology_ml/graph/data.ttl"
    )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--csv", required=True, help="Path to a CSV with at least date/total columns"
    )
    args = ap.parse_args()
