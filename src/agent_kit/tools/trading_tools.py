...
def calculate_indicators(ticker: str, market_data: list[dict]) -> dict:
    ...
    ema_50 = float(_calculate_ema(np.array(prices), 50)[-1]) if len(prices) >= 50 else ema_20
...
@function_tool
def calculate_volatility(prices: list[float], period: int = 20, annualize: bool = True) -> float:
    """
