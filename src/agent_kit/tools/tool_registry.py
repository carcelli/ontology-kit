...
class ToolCategory:
    ...
    # Meta
    ALL = [ONTOLOGY, VECTOR_SPACE, VISUALIZATION, BETTING, TRADING, 
           ML_TRAINING, BUSINESS, GITHUB, SEMANTIC_GRAPH]
...
class ToolRegistry:
    ...
    def _generate_tool_spec(self, function: Callable, metadata: ToolMetadata) -> dict[str, Any]:
...
def register_all_tools():
    ...
    from agent_kit.tools import betting_tools, trading_tools, ml_training, business
    from agent_kit.tools import ontology, vector_space, semantic_graph
    from agent_kit.tools import github_tools, hyperdim_viz, interactive_viz
    
    # Betting tools
    _global_registry.register_tool(
        betting_tools.fetch_odds, ToolCategory.BETTING,
        cost_estimate=0.01, requires_api_key=True, api_provider="odds_api",
        tags=["sports", "odds", "data"]
    )
    _global_registry.register_tool(
        betting_tools.fetch_player_props, ToolCategory.BETTING,
        cost_estimate=0.01, requires_api_key=True, api_provider="odds_api",
        tags=["sports", "props", "player"]
    )
    _global_registry.register_tool(
        betting_tools.detect_arbitrage, ToolCategory.BETTING,
        cost_estimate=0.0, tags=["arbitrage", "analysis"]
    )
    _global_registry.register_tool(
        betting_tools.calculate_implied_probability, ToolCategory.BETTING,
        cost_estimate=0.0, latency_ms=10, tags=["math", "probability"]
    )
    
    # Trading tools
    _global_registry.register_tool(
        trading_tools.fetch_market_data, ToolCategory.TRADING,
        cost_estimate=0.005, requires_api_key=True, api_provider="alpha_vantage",
        tags=["market", "data", "price"]
    )
    _global_registry.register_tool(
        trading_tools.calculate_rsi, ToolCategory.TRADING,
        cost_estimate=0.0, latency_ms=50, tags=["indicators", "technical"]
    )
    _global_registry.register_tool(
        trading_tools.calculate_macd, ToolCategory.TRADING,
        cost_estimate=0.0, latency_ms=50, tags=["indicators", "technical"]
    )
    _global_registry.register_tool(
        trading_tools.calculate_indicators, ToolCategory.TRADING,
        cost_estimate=0.0, latency_ms=100, tags=["indicators", "analysis"]
    )
    _global_registry.register_tool(
        trading_tools.execute_trade, ToolCategory.TRADING,
        cost_estimate=1.0, requires_api_key=True, api_provider="broker",
        tags=["execution", "trading"]
    )
    
    # ML tools (from existing registry)
    _global_registry.register_tool(
        ml_training.train_model, ToolCategory.ML_TRAINING,
        cost_estimate=0.10, latency_ms=5000, tags=["ml", "training"]
    )
    _global_registry.register_tool(
        ml_training.analyze_leverage, ToolCategory.ML_TRAINING,
        cost_estimate=0.05, latency_ms=2000, tags=["ml", "leverage"]
    )
    _global_registry.register_tool(
        ml_training.cluster_data, ToolCategory.ML_TRAINING,
        cost_estimate=0.02, latency_ms=1000, tags=["ml", "clustering"]
    )
    
    # Business tools
    _global_registry.register_tool(
        business.predict, ToolCategory.BUSINESS,
        cost_estimate=0.01, tags=["prediction", "business"]
    )
    _global_registry.register_tool(
        business.optimize, ToolCategory.BUSINESS,
        cost_estimate=0.01, tags=["optimization", "business"]
    )
    
    # Ontology tools
    _global_registry.register_tool(
        ontology.query_ontology, ToolCategory.ONTOLOGY,
        cost_estimate=0.0, latency_ms=50, tags=["sparql", "query"]
    )
    _global_registry.register_tool(
        ontology.add_ontology_statement, ToolCategory.ONTOLOGY,
        cost_estimate=0.0, latency_ms=50, tags=["sparql", "write"]
    )
    
    # Vector space tools
    _global_registry.register_tool(
        vector_space.embed, ToolCategory.VECTOR_SPACE,
        cost_estimate=0.0001, latency_ms=100, tags=["embeddings", "vector"]
    )
    _global_registry.register_tool(
        vector_space.query_vector_index, ToolCategory.VECTOR_SPACE,
        cost_estimate=0.0, latency_ms=50, tags=["search", "vector"]
    )
    
    # Semantic graph tools
    _global_registry.register_tool(
        semantic_graph.build_semantic_graph, ToolCategory.SEMANTIC_GRAPH,
        cost_estimate=0.05, latency_ms=2000, tags=["graph", "semantic"]
    )
    
    # Visualization tools
    try:
        _global_registry.register_tool(
            hyperdim_viz.generate_hyperdim_viz, ToolCategory.VISUALIZATION,
            cost_estimate=0.01, latency_ms=1000, tags=["viz", "hyperdim"]
        )
    except:
        pass
    ...
    # GitHub tools
    _global_registry.register_tool(
        github_tools.write_to_github, ToolCategory.GITHUB,
        cost_estimate=0.0, requires_api_key=True, api_provider="github",
        tags=["github", "write"]
    )
