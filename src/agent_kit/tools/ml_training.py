...
def train_model(
    ...
    return {
        "status": "SCHEDULED",
        "job_id": job_id,
        "message": "Use check_job_status to monitor.",
    }
...
def _finalize_job(job: dict[str, Any]) -> dict[str, Any]:
    ...
    if job["type"] == "TRAIN":
        return {
            "status": "COMPLETED",
            "job_id": job["artifact_uri"]
            .split("_")[-1]
            .replace("TrainedModel-", "train-job-"),
    ...
    return {
        ...
        "job_id": job["artifact_uri"]
        .split("_")[-1]
        .replace("PerformanceMetric-", "cv-job-"),
        "artifact_uri": job["artifact_uri"],
...
class LeverageAnalysisInput(BaseModel):
    ...
    kpi_term: str = Field(
        ..., description="Key Performance Indicator for sensitivity analysis"
    )
...
ML_TOOL_REGISTRY = {
    "train_model": {
        ...
        "tool_spec": pydantic_to_openai_tool(
            "train_model",
            "Schedule model training and return a job_id.",
            ModelTrainingInput,
        ),
    },
...
class ClusteringInput(BaseModel):
    ...
    data: list[list[float]] = Field(
        ..., description="2D array of data points to cluster"
    )
    eps: float = Field(
        default=0.5,
        gt=0.0,
        description="Maximum distance between points in cluster (DBSCAN)",
    )
    min_samples: int = Field(
        default=5, ge=1, description="Minimum points to form dense region (DBSCAN)"
    )
    algorithm: str = Field(
        default="DBSCAN",
        description="Clustering algorithm: DBSCAN, KMeans, or Hierarchical",
    )
    n_clusters: int | None = Field(
        None, description="Number of clusters (for KMeans/Hierarchical)"
    )
...
try:
    ...
    logger.warning(
        "Interactive visualization tools not available (install plotly, kaleido)"
    )
