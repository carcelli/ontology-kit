...
class SemanticGraphInput(BaseModel):
    ...
    corpus_path: str | None = Field(
        None,
        description="Optional text corpus to extract relations (drives, depends_on, etc.)",
    )
...
class TargetLeverageInput(BaseModel):
    ...
    top_k: int = Field(
        default=5, ge=1, le=50, description="Number of top levers to return"
    )
...
class InterventionRecommendationInput(BaseModel):
    ...
    historical_data_path: str | None = Field(
        None,
        description="Optional historical experiment data for effect size estimates",
    )
    top_paths: int = Field(
        default=3, ge=1, le=10, description="Number of paths to analyze"
    )
...
def build_semantic_graph(input_data: SemanticGraphInput) -> dict[str, Any]:
    ...
    logger.info(
        "Building semantic graph: %s with %d terms", job_id, len(input_data.terms)
    )
...
def recommend_interventions(
    input_data: InterventionRecommendationInput,
) -> dict[str, Any]:
    """
    ...
    logger.info(
        "Generating interventions for: %s → %s", input_data.node, input_data.target
    )
