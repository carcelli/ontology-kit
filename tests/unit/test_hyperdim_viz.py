...
def test_generate_viz_from_terms(sample_terms, temp_output):
    ...
    result_path = generate_hyperdim_viz(
        terms=sample_terms, output_file=temp_output, n_components=2
    )
...
def test_generate_viz_3d(sample_terms, temp_output):
    ...
    result_path = generate_hyperdim_viz(
        terms=sample_terms, output_file=temp_output, n_components=3
    )
...
def test_generate_viz_from_ontology(temp_output):
    ...
    result_path = generate_hyperdim_viz(
        ontology_path=ontology_path, output_file=temp_output
    )
...
def test_terms_override_ontology(sample_terms, temp_output):
    ...
    result_path = generate_hyperdim_viz(
        ...
        terms=sample_terms,
        output_file=temp_output,  # Should use these, not ontology
    )
...
def test_invalid_n_components(sample_terms, temp_output):
    ...
    with pytest.raises(ValueError, match="must be 2 or 3"):
        generate_hyperdim_viz(
            terms=sample_terms, n_components=4, output_file=temp_output
        )
...
def test_nonexistent_ontology(temp_output):
    ...
    with pytest.raises(FileNotFoundError):
        generate_hyperdim_viz(
            ontology_path="/nonexistent/path.ttl", output_file=temp_output
        )
...
def test_custom_perplexity(sample_terms, temp_output):
    ...
    result_path = generate_hyperdim_viz(
        terms=sample_terms, output_file=temp_output, perplexity=5
    )
...
def test_max_terms_limit(temp_output):
    ...
    result_path = generate_hyperdim_viz(
        terms=many_terms[:10], output_file=temp_output, max_terms=10
    )  # Limit to 10
